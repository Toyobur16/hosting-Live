      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-[#f1f5f9]">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>{lang === 'bn' ? 'বট ব্যবহারকারী' : 'Bot Users'} ({users.length})</span>
          </button>
          ...
          >
            ...
            <span>{lang === 'bn' ? 'উত্তোলন অনুরোধ' : 'Withdrawals'} ({withdraws.length})</span>
          </button>
        </div>
              ...
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-[#94a3b8]">
                    {lang === 'bn' ? 'কোনো ব্যবহারকারী পাওয়া যায়নি।' : 'No users registered yet.'}
                  </td>
                </tr>
              ) : (
                filteredUsers.map((u) => (
              ...
              {withdraws.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-[#94a3b8]">
                    {lang === 'bn' ? 'কোনো উইথড্র অনুরোধ পাওয়া যায়নি।' : 'No withdrawal requests found.'}
                  </td>
                </tr>
              ) : (
                withdraws.map((w) => (
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 max-w-sm w-full shadow-xl">
            <h4 className="text-sm font-bold text-[#1e293b] mb-2 flex items-center gap-2">
              ...
              {lang === 'bn' ? 'ব্যবহারকারীর ব্যালেন্স পরিবর্তন' : 'Adjust User Balance'}
            </h4>
            ...
            <div className="mb-4">
              <label className="block text-xs font-bold uppercase tracking-wider text-[#64748b] mb-1.5">
                {lang === 'bn' ? 'পরিমাণ যোগ/বিয়োগ (+ অথবা -)' : 'Amount to add/deduct (e.g. 50 or -20):'}
              </label>
              <input
                type="number"
